...
const Servicos = () => {
    ...
    const { data: prestadores, isLoading, error } = useBuscaPrestadores({
    ...
    const handleSearch = () => {
        ...
        setSearchParams({
            ...
            ...(filtros.avaliacaoMin && { avaliacaoMin: String(filtros.avaliacaoMin) }),
        });
    };
    ...
    const handleFilterChange = (key: keyof typeof filtros, value: number | undefined) => {
        ...
        <div className="min-h-screen bg-background">
            ...
            <div className="container mx-auto px-4 py-8 flex flex-col md:flex-row gap-8">
                ...
                <aside className="w-full md:w-64 shrink-0">
                    <div className="bg-card border border-border rounded-lg p-6 sticky top-24 space-y-6">
                        ...
                        <FiltroSelect
                            ...
                            options={avaliacaoOptions.map((a) => ({ id: a, nome: `${a}+ estrelas` }))}
                            placeholder="Todas"
                ...
                <div className="flex-1 space-y-4">
                    ...
                    {error && <p className="text-red-500">Erro ao carregar: {error.message}</p>}
                    ...
                    {!location && !filtros.searchText && !filtros.servicoId && (
                            <div className="p-4 bg-yellow-100 border border-yellow-400 text-yellow-700 rounded-lg">
                                Permita a localização para ver a distância e prestadores mais próximos.
                            </div>
                    )}
                    {prestadores?.length === 0 && !isLoading && <p>Nenhum prestador encontrado.</p>}
                    {prestadores?.map((p) => (
                        <ProviderCard
                            key={p.id}
                            name={p.nomeUsuario}
