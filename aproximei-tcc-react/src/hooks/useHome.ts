...
export const useBuscaPrestadores = (params: BuscaParams) => {
    // A query só é ativada se houver pelo menos um parâmetro relevante (palavra, ID, avaliação ou localização)
    const enabled = !!params.palavra || !!params.servicoId || !!params.categoriaId || !!params.avaliacaoMin || (!!params.latitude && !!params.longitude);
